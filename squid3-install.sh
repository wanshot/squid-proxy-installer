...
if cat /etc/os-release | grep PRETTY_NAME | grep "Ubuntu 18.04"; then
    ...
    /usr/bin/wget --no-check-certificate -O /etc/squid/squid.conf https://raw.githubusercontent.com/wanshot/squid-proxy-installer/master/conf/squid.conf
    ...
    /usr/bin/wget --no-check-certificate -O /etc/squid3/squid.conf https://raw.githubusercontent.com/wanshot/squid-proxy-installer/master/conf/squid.conf
    /sbin/iptables -I INPUT -p tcp --dport 3128 -j ACCEPT
    /sbin/iptables-save
